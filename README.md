...
- **Filters & reports** — date, status, optional course filter and search all combine; the loaded register prints to a branded **PDF**. Each student name opens a **per-student attendance page** (today / yesterday / this week / this month / all time + status filter) with its own compact overview and PDF report. "Marked by" always shows the staff member's role (or *Biometric*).
